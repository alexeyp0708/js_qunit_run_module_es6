...
function run( args, options ) {
    ...
    let runFilesPromise=new Promise( (res)=>{
        res();
    } );
    for(let i = 0; i < files.length; i++){
        let filePath = path.resolve( process.cwd(), files[ i ] );
        delete require.cache[ filePath ];
        if(/^win/i.test(process.env.OS)){
            filePath=filePath.replace(/[\\]+/g,'/');
            if(/^\D:[/|\\]/i.test(filePath)){
                filePath='/'+filePath;
            }
        }
        runFilesPromise=runFilesPromise.then( ()=>{
    ...
    runFilesPromise.then(()=>{
        console.log('lst operation');
        let running = true;
